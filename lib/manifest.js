...
// and blocks tools/call for tools not in the manifest or with drifted schemas.
...
export function buildManifest(tools, serverInfo = null, spec = null) {
  const entries = tools.map((t) => ({
    name: t.name,
    schemaHash: hash(t.inputSchema ?? {}),
    descriptionHash: hash(t.description ?? ""),
  }));
  const manifestHash = hash(entries.map((e) => `${e.name}:${e.schemaHash}`));
  return {
    version: 1,
    spec,
    serverInfo,
