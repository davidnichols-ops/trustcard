...
import { changeVector } from "./change.js";
import { implementationIdentity } from "./descriptor.js";
import { REASONS } from "./trust.js";

export class TrustSession {
  constructor({ cmd, args, env, trust, guard, protocolVersions, onEvent, implementation = null, publisherKeyId = null } = {}) {
    ...
    // v2 (additive): implementation identity + publisher key for this provider,
    // so refresh() can compute a full changeVector (interface + implementation +
    // provenance axes), not just the interface diff. Both default to "unresolved".
    this.implementation = implementationIdentity(implementation);
  ...
  async refresh(reason = "manual") {
    ...
    this.previousObservation = prev;
    this.observation = next;

    // v2 (additive): full change vector across interface + implementation +
    // provenance axes. Same implementation/publisher on both sides (they are
    // session-level), so this surfaces interface+permission movement here; the
    // implementation axis becomes decisive when callers re-observe a swapped
    // artifact (see change.js and test/change.test.js).
    const vector = changeVector(
      { tools: prev.tools, implementation: this.implementation, publisherKeyId: this.publisherKeyId },
      { tools: next.tools, implementation: this.implementation, publisherKeyId: this.publisherKeyId }
    );

    const evalResult = this.trust?.evaluate(this.serverId, next, diff);
    const event = { type: "refresh", reason, diff: diff.summary, action: evalResult?.action, vector: vector.vector };
    this.onEvent(event);
    ...
    return { diff, vector, evaluation: evalResult, observation: next, previous: prev };
  }
